...
class User(db.Model):
    ...
    def to_dict(self):
        try:
            skills_list = json.loads(self.skills or "[]")
        except Exception:
            skills_list = []
        return {
            "id": self.id,
            ...
            "experience": self.experience or "Mid-level",
...
class Interview(db.Model):
    ...
    questions     = db.relationship("Question", backref="interview", lazy=True, order_by="Question.id")

    def to_dict(self, include_questions=False):
        try:
            topics_list = json.loads(self.topics or "[]")
        except Exception:
            topics_list = []
        
        duration = None
        if self.completed_at and self.started_at:
            duration = max(1, int((self.completed_at - self.started_at).total_seconds() / 60))
        ...
        d = {
            ...
            "topics": topics_list,
            "status": self.status,
            "started_at": self.started_at.isoformat() if self.started_at else datetime.utcnow().isoformat(),
        ...
        if include_questions:
            ...
            try:
                d["report"] = json.loads(self.report or "{}")
            except Exception:
                d["report"] = {}
            try:
                d["rag_context"] = json.loads(self.rag_context or "[]")
            except Exception:
                d["rag_context"] = []
        return d
...
class Question(db.Model):
    ...
    def to_dict(self):
        try:
            fb = json.loads(self.feedback or "{}")
        except Exception:
            fb = {}
        return {
            "id": self.id,
            "question_text": self.question_text,
            "question_type": self.question_type or "technical",
            "topic": self.topic or "",
            "answer_text": self.answer_text or "",
            "score": self.score or 0,
            "feedback": fb,
            "model_answer": self.model_answer or "",
            "rag_source_id": self.rag_source_id or "",
            "asked_at": self.asked_at.isoformat() if self.asked_at else datetime.utcnow().isoformat(),
        }


def _get_current_user():
    """Helper to safely retrieve current user from JWT identity."""
...
@jwt_required()
def upload_resume():
    """Accept plain-text resume, extract skills via Watsonx/RAG, save to profile."""
    user = _get_current_user()
    if not user:
        return jsonify({"error": "User not found"}), 404
    d = request.get_json() or {}
    ...
    from watsonx_client import analyze_resume

    matched_questions = search_by_resume_keywords(resume_text, user.role)
    inferred_categories = list({q["category"] for q in matched_questions})
    
    # Try getting extra insights from Granite
    insights = analyze_resume(resume_text, user.role)
    granite_skills = insights.get("skills", [])
    
    # Combined unique skills
    combined_skills = list(dict.fromkeys(inferred_categories + [s for s in granite_skills if isinstance(s, str)]))

    user.resume_text = resume_text
    user.skills = json.dumps(combined_skills)
    db.session.commit()

    return jsonify({
        "message": "Resume processed successfully",
        "inferred_skills": combined_skills,
        "matched_question_count": len(matched_questions),
        "experience_years": insights.get("experience_years", 3),
        "readiness_score": insights.get("readiness_score", 75),
        "user": user.to_dict(),
    }), 200
...
def get_industry_profile():
    ...
    context = retrieve_industry_context(role)
    return jsonify({"context": context, "role": role}), 200
...
def preview_questions():
    ...
    user = _get_current_user()
    d = request.get_json() or {}
    role       = d.get("role", user.role if user else "Software Engineer")
    difficulty = d.get("difficulty")
    topics     = d.get("topics", [])
    resume_kw  = user.resume_text if user else ""
    ...
    if not questions or len(questions) < 4:
        for topic in (topics or ["General", "Architecture", "Problem Solving"]):
            questions += retrieve_questions(role=role, difficulty=difficulty, category=topic, limit=4)
        if not questions:
            questions = retrieve_questions(role=role, difficulty=difficulty, limit=8)
...
def start_interview():
    ...
    rag_context = [{"id": r["id"], "category": r["category"], "keywords": r.get("keywords", [])} for r in rag_items]
...
def get_question(session_id):
    ...
    topic = req_data.get("topic", topics[len(interview.questions) % len(topics)] if topics else "General")
    ...
    if rag_items:
        best = rag_items[0]
        rag_source_id = best.get("id", "")
        model_answer_hint = best.get("model_answer", "")
        rag_context_str = (
            f"\nRelevant industry context:\n"
            f"- Topic focus area: {best.get('category', topic)}\n"
            f"- Industry note: {best.get('industry_context', '')}\n"
            f"- Target keywords to probe: {', '.join(best.get('keywords', []))}\n"
        )
...
def submit_answer(session_id):
    ...
    answer_text = d.get("answer", "").strip()

    question = db.session.get(Question, question_id)
    if not question or question.interview_id != interview.id:
        return jsonify({"error": "Question not found in this interview session"}), 404
    ...
    question.score = float(evaluation.get("score", 5))
    ...
    return jsonify({
        ...
        "model_answer": question.model_answer or evaluation.get("sample_answer", ""),
    }), 200
...
def complete_interview(session_id):
    ...
    experience = user.experience if user else "Mid-level"
    ...
    interview.grade        = str(report.get("grade", "C"))
    interview.report       = json.dumps(report)
    db.session.commit()

    return jsonify({"report": report, "interview": interview.to_dict(include_questions=True)}), 200
...
def get_interview(session_id):
    interview = Interview.query.filter_by(session_id=session_id).first_or_404()
    return jsonify({"interview": interview.to_dict(include_questions=True)}), 200
...
def list_interviews():
    ...
    if not user:
        return jsonify({"error": "User not found"}), 404
    interviews = Interview.query.filter_by(user_id=user.id).order_by(
...
def get_analytics():
    ...
    trend  = [
        {"date": i.completed_at.strftime("%b %d") if i.completed_at else "Recent", "score": i.overall_score}
        for i in sorted(completed, key=lambda x: x.started_at or datetime.min)[-10:]
    ]
    by_diff = {}
    for i in completed:
...
def get_questions():
    ...
    if q_type:
        questions = [q for q in questions if q["type"].lower() == q_type.lower()]
    if role:
        questions = [q for q in questions if q["role"].lower() == role.lower() or q["role"] == "All"]
    if search:
        s = search.lower()
        questions = [
            ...
            or any(s in kw.lower() for kw in q.get("keywords", []))
        ]
